...
public class AccountController {
    ...
    public ResponseEntity<String> createUser(HttpServletRequest request) {
        ...
        if (!messages.isEmpty()) {
            ...
            return new ResponseEntity<String>(sb.toString(), message,
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }

        // create user account
        try {
            // accountService.createAccount(display_name, password_confirm,
            // email, first_name, last_name);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<String>("Error Creating Account", message,
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
